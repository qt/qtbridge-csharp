...
namespace Qt.DotNet.CodeGeneration.MetaFunctions
{
    ...
    public class BasicTypes : MetaFunction
    {
        protected override string Eval(object src, Enum traits) => src switch
        {
            Type type when type.IsBuiltIn() => type switch
            {
                ...
                _ when type.Is<nint>() => "void *",
                _ when type.Is<nuint>() => "void *",
                _ when type.Is<float>() => "float",
                _ when type.Is<double>() => "double",
                _ when type.Is<bool>() => "bool",
