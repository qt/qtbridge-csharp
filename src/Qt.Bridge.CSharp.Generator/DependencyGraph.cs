...
namespace Qt.Bridge.CodeGeneration
{
    ...
    public class DependencyGraph : IReadOnlyDictionary<Type, ConcurrentSet<MemberInfo>>
    {
        ...
        private Assembly ApiAssembly => lazy.Get(() => ApiAssembly, ()
            => GetAssembly("Qt.Bridge.CSharp.Api"));
        ...
        private bool IsBuiltIn(Type type)
        {
            ...
            if (type.IsPrimitive)
                return true;
            if (type.Assembly == AdapterAssembly || type.Assembly == ApiAssembly)
                return true;
            return false;
        }
        ...
        private bool IsValidMember(MemberInfo i)
        {
            ...
            if (i.DeclaringType?.Assembly == AdapterAssembly)
                return false;
            if (i.DeclaringType?.Assembly == ApiAssembly)
                return false;
            if (i.IsOverrideOf(AdapterAssembly) || i.IsOverrideOf(ApiAssembly))
                return false;
            return true;
        }
